...
public class BeansGroupList {
    public List<Student> getGroupList(
        ...
        query = addQueryPart(query, "groupEducationForm", groupEducationFormQuery);
        query = addQueryPart(query, "groupQualificationLevel", groupQualificationLevelQuery);
        query = addQueryPart(query, "groupNumber", groupNumberQuery);
        query = addQueryPart(query, "groupCourse", groupCourseQuery);
        query = addQueryPart(query, "groupStatus", groupStatusQuery);
    ...
    public HashMap<String, Boolean> getGroupEducationFormListForTitle() {
        ...
        try {
            ...
            for (int i = 0; i < orderList.size(); i++) {
                storage.put(orderList.get(i).getGroupEducationForm(), false);
            }
    ...
    public HashMap<String, Boolean> getGroupQualificationLevelListForTitle() {
        ...
        try {
            ...
            for (int i = 0; i < orderList.size(); i++) {
                storage.put(orderList.get(i).getGroupQualificationLevel(), false);
            }
    ...
    public HashMap<String, Boolean> getGroupStatusListForTitle() {
        ...
        try {
            ...
            for (int i = 0; i < orderList.size(); i++) {
                storage.put(orderList.get(i).getGroupStatus(), false);
            }
