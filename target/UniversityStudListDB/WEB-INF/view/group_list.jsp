                    ...
                    <ul>
                        <c:forEach var="groupSpecialityList" items="${groupSpecialityList}">
                            <c:if test="${groupSpecialityList.value eq true}">
                                <li><h6><input type="checkbox" name="groupSpec=${groupSpecialityList.key}" value="${groupSpecialityList.key}"
                    ...
                    <ul>
                        <c:forEach var="groupEducationFormList" items="${groupEducationFormList}">
                            <c:if test="${groupEducationFormList.value eq true}">
                                <li><h6><input type="checkbox" name="groupEducForm=${groupEducationFormList.key}" value="${groupEducationFormList.key}"
                            ...
                            <c:if test="${groupEducationFormList.value eq false}">
                                <li><h6><input type="checkbox" name="groupEducForm=${groupEducationFormList.key}"
                                               value="${groupEducationFormList.key}"> ${groupEducationFormList.key} </h6></li>
                            </c:if>
                        </c:forEach>
                    </ul>
                    <hr>
                    ...
                    <ul>
                        <c:forEach var="groupQualificationLevelList" items="${groupQualificationLevelList}">
                            <c:if test="${groupQualificationLevelList.value eq true}">
                                <li><h6><input type="checkbox" name="groupQualLevel=${groupQualificationLevelList.key}" value="${groupQualificationLevelList.key}"
                            ...
                            <c:if test="${groupQualificationLevelList.value eq false}">
                                <li><h6><input type="checkbox" name="groupQualLevel=${groupQualificationLevelList.key}"
                                               value="${groupQualificationLevelList.key}"> ${groupQualificationLevelList.key} </h6></li>
                            </c:if>
                        </c:forEach>
                    </ul>
                    <hr>
                    ...
                    <ul>
                        <c:forEach var="groupNumberList" items="${groupNumberList}">
                            <c:if test="${groupNumberList.value eq true and groupNumberList.key ne null}">
                                <li><h6><input type="checkbox" name="groupNumber=${groupNumberList.key}" value="${groupNumberList.key}"
                    ...
                    <ul>
                        <c:forEach var="groupCourseList" items="${groupCourseList}">
                            ...
                            <c:if test="${groupCourseList.value eq false and groupCourseList.key ne null}">
                                <li><h6><input type="checkbox" name="groupCourse=${groupCourseList.key}"
                                               value="course=${groupCourseList.key}"> ${groupCourseList.key} курс</h6></li>
                            </c:if>
                        </c:forEach>
                    </ul>
                    <hr>
            ...
            <div class="container">
                <table id="stupid" class="table table-hover font-table">
                    ...
                    <c:forEach var="groupList" items="${groupList}">
                        <tr>
                            <td><a href="group_info?order_id=${groupList.id}">${groupList.groupNumber}</a></td>
                            <td><a href="group_info?order_id=${groupList.id}">${groupList.groupEducationForm}</a></td>
                            <td><a href="group_info?order_id=${groupList.id}">${groupList.groupQualificationLevel}</a></td>
                            <td><a href="group_info?order_id=${groupList.id}">${groupList.groupCourse}</a></td>
                            <td><a href="group_info?order_id=${groupList.id}">${groupList.groupStatus}</a></td>
                        </tr>
                    </c:forEach>
